...
- [**Python**](#python)
  ...
  - [Базовые типы данных](#базовые-типы-данных)
    - [Строки](#строки)
      - [Строки и операции над ними](#строки-и-операции-над-ними)
  ...
  - [ООП](#ооп)
    ...
    - [Функции для работы с классами](#функции-для-работы-с-классами)
...
- [Фреймворки](#фреймворки)
  - [Django](#django)
    ...
    - [Миграции](#миграции)
    - [Извлечение объектов из модели](#извлечение-объектов-из-модели)
  - [Flet](#flet)
  - [Pytest](#pytest)
  - [FastApi](#fastapi)
...
- [GIT](#git)
  ...
  - [Некоторые полезные команды терминала](#некоторые-полезные-команды-терминала)
  - [Содержимое директории .git](#содержимое-директории-git)
  - [Cообщения, которые вы можете встретить при проверке статуса:](#cообщения-которые-вы-можете-встретить-при-проверке-статуса)
...
- `str.zfill()` - вернет копию строки, у которой начало строки будет заполнено цифрой ASCII 0, до указанной длины width.
...
`a.intersection(b)` - Возвращает множество, состоящее из элементов, которые встречаются и в set, и в other.
...
`set.symmetric_difference(other)` - Симметричная разность.	Позволяет исключить из результата общие элементы для множества и последовательности. Метод возвращает новое множество, в котором нет одинаковых элементов, встречающихся одновременно в set и итерируемом объекте other.
...
- `getattr()` -	Возвращает значение атрибута экземпляра или класса

- `setattr()` -	Устанавливает или изменяет значение атрибута экземпляра или класса
- `hasattr()` -	Проверяет наличие атрибута в экземпляре или классе
- `delattr()` -	Удаляет локальные атрибуты экземпляра или атрибуты класса
- `isinstance()` - Выполняет проверку соответствия типа объекта и указанного класса
...
[Все типы полей находятся на странице](https://docs.djangoproject.com/en/4.1/ref/models/fields/)
...
[Все предоставляемые веб-фреймворком Django конверторы путей можно посмотреть по адресу](https://docs.djangoproject.com/en/4.1/topics/http/urls/#path-converters)
...
- `python manage.py makemigrations <имя приложения>` - создает миграцию. Имя приложения необязательно писать
...
- `python manage.py createsuperuser` - создает пользователя для работы с администрированием сайта
- `python manage.py shell` - открывает внутренную оболочку python в терминале
...
Meta class - это класс который определяет метаданные модели. Атрибут `ordering` сообщает Django по какому полю он должен сортировать результаты. Опция `indexes` - позволяет определить в модели индексы базы данных, который могу содержать одно или несколько полей в возрастающем либо убывающем порядке.
...
1. `from django.utils import timezone`
   - `timezone.now` - метод возвращает текущую дату/время в формате, зависящего от часового пояса
      ```
      publish = models.DateTimeField(default=timezone.now)
      ```
2. `from django.contrib.auth.models import User`
   - Это поле определяет взаимосвязь многиек-одному, означающую, что каждый пост написан пользователем и пользователь может написать любое число постов. этого поля Django создаст внешний ключ в базе данных, используя первичный ключ соответствующей модели. Параметр on_delete определяет поведение, которое следует применять при удалении объекта, на который есть ссылка. Это поведение не относится конкретно к Django; оно является стандартным для SQL. Использование ключевого слова CASCADE указывает на то, что при удалении пользователя, на которого есть ссылка, база данных также удалит все связанные с ним пос ты в блоге
...
Применение метода filter() - для фильтрации запросов QuerySet:
```
>>> Post.objects.filter(publish__year=2022, author__username='admin')
```

Применение метода exclude() - для того чтобы исключить из набора запросов QuerySet:
```
...
`GitHub` — веб-сервис, который основан на системе Git. Его часто называют «социальной сетью для разработчиков», которая помогает удобно вести коллективную разработку IT-проектов. Здесь можно публиковать и редактировать свой код, комментировать чужие наработки, следить за новостями других пользователей. У программистов есть возможность объединять репозитории и выводить вклад каждого участника в определённый репозиторий в виде дерева.

`Git` — довольно гибкая система, и в ней есть множество настроек. Но мы начнём с необходимых. Для начала установим имя пользователя и адрес электронной почты. Эти данные будут отображены в каждом изменении, которое вы вносите, и другие пользователи смогут понять, кто автор тех или иных изменений.
...
- **Область подготовленных файлов** — это файл, располагающийся в Git директории, в котором содержится информация о том, какие изменения попадут в следующий коммит. Эту область ещё называют «индекс».
- **Git-репозиторий (Git directory/repository)** — это каталог, в котором Git хранит метаданные и базу данных объектов вашего проекта. Именно он копируется, когда вы клонируете репозиторий с другого компьютера. Только эта часть содержится, когда вы только что создали новый Git-репозиторий или клонировали существующий с удалённого компьютера.
...
Перейдите [по ссылке](https://git-scm.com/), откройте раздел Downloads (Загрузки), выберите операционную систему вашего компьютера и кликните соответствующую ссылку для загрузки приложения.

Комментарии для других операционных систем:

1. LINUX
- Если вы никогда не пользовались терминалом, воспользуйтесь следующей инструкцией, чтобы его открыть.
- Откройте терминал и установите приложение при помощи пакетного менеджера вашего дистрибутива. Для Ubuntu команда будет выглядеть так: `sudo apt-get install git`

2. OS X
- Откройте терминал. Если вы никогда им пользовались, воспользуйтесь следующей инструкцией, чтобы его открыть.
- Для установки Git проще всего воспользоваться homebrew. Этот инструмент устанавливает нужные вам пакеты, не предоставляемые Apple. Если он не установлен, воспользуйтесь инструкцией или информацией здесь.
- После установки homebrew откройте терминал и запустите команду: `brew install git`

Также может быть полезно: [Как настроить терминал на Mac](https://yablyk.com/642080-kak-nastroit-terminal-v-mac-macos-i-sdelat-ego-bolee-poleznym/).

`Терминал` — текстовый интерфейс. Чтобы им пользоваться, нужно знать специальные команды, которые надо будет вводить с клавиатуры.

После того как все действия по установке завершены, давайте проверим, что Git появился в системе компьютера. Откройте терминал и введите git --version.

```
blahblahs-MacBook-Air:~ blahblah$ git --version
git version 2.26.2
```
...
## Некоторые полезные команды терминала

Для того чтобы пользоваться Git, очень полезно освоить базовые команды терминала [UNIX-систем](https://www.dmosk.ru/terminus.php?object=unix):
...
- `git status` - Статус репозитория показывает информацию о текущем состоянии репозитория: есть ли что-то новое в репозитории, изменились ли какие-то файлы, и если да, то какие, и так далее.
- `git branch` - Посмотреть, в какой вы ветке
- `git branch НазваниеВетки` - создать новую ветку
- `git checkout НазваниеВетки` - Чтобы перейти на существующую ветку
- `git commit -m 'описать что вы делали'` коммит — это состояние репозитория в определённый момент времени. Именно к коммиту мы можем возвращаться, чтобы посмотреть состояние объектов в какое-то время.
- `git add -A` - Чтобы добавить все файлы, которые находятся в директории
- `git add this.js` -  чтобы добавлять отдельные файлы, используйте ту же команду add, но с указанием имени файла
- `git commit --amend 'описать что делали'` - Выбранные изменения будут добавлены в последний коммит. Команда также вызовет открытие системного текстового редактора для внесения изменений в сообщение коммита. Эту команду часто используют, если была допущена ошибка.
- `git merge НазваниеВетки` - Слияние — это способ соединить обратно раздвоенную историю. Команда `git merge` позволяет взять независимые части разработки и интегрировать их в одну ветку.
...
## Cообщения, которые вы можете встретить при проверке статуса:
...
- `Working tree clean` - дословно означает, что рабочий каталог не содержит изменённых файлов. Система контроля версий Git использует так называемую архитектуру трёх деревьев. Working tree связана с рабочей директорией Git, в которой содержатся рабочие версии файлов.
...
- [Гид по изучение Git от Atlassian](https://www.atlassian.com/git/tutorials/learn-git-with-bitbucket-cloud).
- [Шпаргалка](https://github.github.com/training-kit/downloads/ru/github-git-cheat-sheet/).
- [Git Handbook](https://guides.github.com/introduction/git-handbook/).
- Поиграть с визуализацией можно [здесь](http://git-school.github.io/visualizing-git/).
- Обязательно поупражняйтесь в [этом тренажёре](https://learngitbranching.js.org/).
