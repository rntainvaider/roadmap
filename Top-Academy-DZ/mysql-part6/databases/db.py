...
class DbWorker:
    ...
    def update_user(self, user_id: int, first_name=None, last_name=None, age=None, gender=None, email=None) -> None:
        with Session(autoflush=False, bind=self.engine) as bd:
            user = bd.query(User).filter(User.id == user_id).first()
            if user:
                if first_name:
    ...
    def search_user(self, first_name: str, last_name: str) -> None:
        with Session(autoflush=False, bind=self.engine) as bd:
            ...
            if users:
                for user in users:
                    print(f"ID: {user.id}, Name: {user.first_name} {user.last_name}, Age: {user.age}, Gender: {user.gender}, Email: {user.email} ")
            else:
                print("Пользователя не существует!")
